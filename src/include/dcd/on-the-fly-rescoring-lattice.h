// on-the-fly-rescoring-lattice.h
...
// \file
// Lattice that implements on-the-fly rescoring (not finished)

#ifndef DCD_ON_THE_FLY_LATTICE_H__
#define DCD_ON_THE_FLY_LATTICE_H__
...
class OnTheFlyRescoringLattice {
 public:
  template<class S>
  struct CoHypothesis {
    S state;
  ...
  template<class S>
  struct CoHypothesisSet {
    int best_index_;
    int worst_index_;
    int max_;
    void Push(const CoHypothesis<S>& hyp) {
      if (hyp.Cost() < WorstCost()) 
        return;
      if (hyp.cost < BestCost(best_index_)) {
      }
    }

    //Perform a quick select across both arrays to get the nth best
    //CoHypothesis
    float FindNthCost(const CoHypothesis<S>& a, const CoHypothesis<S>& b) {
      return kMaxCost;
    }

    void Merge(const CoHypothesis<S>& other) {
      if (Size() == other.Size())
        if (Hash() == other.Hash()) {
          return;
        }
      float threshold = FindNth(*this, other);
      //Swap in hyps other
      //return best;
    }

    float WorstCost() const {
      return data_[worst_index_];
    }

    float BestCost() const { 
      best_index_ == -1 ? kMaxCost : data_[best_index_].Cost();
    }

    //Use OpenFst order independant hash
    size_t Hash() const {
      return 0;
    }
  ...
  struct LatticeState {
   ...
   protected:
    ...
    friend class OnTheFlyRescoringLattice;
  };


  explicit OnTheFlyRescoringLattice(const SearchOptions& opts, ostream* logstream = &std::cerr) 
    : logger_("Lattice", *logstream),
    next_id_(0), num_allocs_(0), num_frees_(0), 
    use_pool_(opts.use_lattice_pool) { }

  virtual ~OnTheFlyRescoringLattice() { 
    Clear();
 ...
 protected:
  ...
  bool use_pool_;
 private:
  DISALLOW_COPY_AND_ASSIGN(OnTheFlyRescoringLattice);
};
} //namespace dcd

#endif //DCD_ON_THE_FLY_LATTICE_H__
