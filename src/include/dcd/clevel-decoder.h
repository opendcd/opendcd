...
// a class this is often more efficient than destroying and allocating
// new classes
...
// to operate on any weight semiring, optional time paramter allow certain
// weigths and arcs to be controlled at specific times.
...
struct ArcExpandOptions {
  ArcExpandOptions(float best, float threshold, float lbest,
                   float lthreshold, T* tokens, T* scratch,
...
class CLevelDecoder {
 public:
  ...
  class SearchArc {
   public:
    ...
    //  If cost to set the first token is less
    ...
    // Returns the best active token cost from nside the arc;
   ...
   protected:
    //  Extra token for entry token e.g. Token tokens_[kTokensPerArc + 1];
    //  TODO allow for various token containers such as vectors
  ...
  // Comparisons for sorting the active arcs
   ...
   public:
    ...
    bool Init(const F& fst, int state, const T& trans_model,
      ...
      for (ArcIterator<F> aiter(fst, state); !aiter.Done(); aiter.Next()) {
        ...
        //  we enter the state. Answer: Doesn't seem to work well
        float exit_weight = trans_model.GetExitWeight(arc.ilabel);
        arcs.push_back(SearchArc(arc.ilabel, arc.olabel,
                                 Value(arc.weight) + opts.insertion_penalty,
                                 arc.nextstate, exit_weight, num_states));
      }
      return true;
    }
    ...
    float ExpandEpsilonArcs(ActiveStateVector* active_states, EpsQueue* q,
                            CLevelDecoder* decoder, float threshold,
                            const SearchOptions& opts) {
      float best = kMaxCost;
      int num_activated = 0;
      for (int i = 0; i != eps_arcs_.size(); ++i) {
        SearchArc* sa = &eps_arcs_[i];
        //  Pass null pointer as epsilons are never in the active arcs lists
        sa->SetEntryToken(token_, threshold, -1, 0, opts);
        Token token = sa->GetExitToken();
        //  TODO maybe not prune until adding a rescoring cost in the state 
        //  expansion
        if (token.Cost() < threshold) {
          pair<SearchState*, float> p =
              decoder->ExpandToFollowingState(sa, threshold);
    ...
    //  Returns the number of epsilon like transitions associated with the state
    int NumEpsilons() const { return eps_arcs_.size(); }

    //  Returns the number of emitting transitions leaving the state
    ...
    //  Set the token in the state and activates if necessary if active and
    //  lower cost, updates the cost
    //  TODO: Maybe incorporating the best scoring arc and pruning on that 
    //  could help
    ...
    //  Resets the search state back to the default uninitialized state. Called 
    //  before returning the search state back to a memory pool
    void Clear() {
      arcs_.clear();
      eps_arcs_.clear();
    ...
    //  When caching a pointer in the SearchArcs we need to know how many
    //  arcs are referencing the state. Only when the ref_count_ is zero is i
    //  safe to collect a search state
    int RefCount() const {
      return ref_count_;
    }
 ...
 public:
  ...
  // Lattice debugging feature to dump the current traceback to an fst,
  // graphviz can be used to then visualize the lattice
  ...
  // Go through the tokens and check the lattice states are in the used lists
  bool GcCheck() {
    lattice_->Check();
    lattice_->SortLists();
    for (int i = 0; i != active_arcs_.size(); ++i) {
  ...
  // Prepare decoder for search and add the initial state to active lists
  // Will return false if the fst doesn't have a valid start state or the
  // active state list is empty
  bool BeginDecode() {
    PROFILE_FUNC();
    int s = fst_->Start();
    if (s == kNoStateId)
    ...
    Token token(ls, 0);  // Create token associated with the start
                         // lattice state and set the cost to zero.
  ...
  ArcExpandResults ExpandActiveArcs_ArcExpansion(int begin, int end) {
    ...
    ArcExpandOptions<Token, L> opts(kMaxCost, kMaxCost, kMaxCost, kMaxCost, 0,
                                    scratch, search_opts_, lattice_);
      ...
      if (arc->GetBestCost() <= threshold_) {
        ...
        if (exit_cost < kMaxCost) {
          ...
          if (exit_cost < threshold_) {
            ...
            pair<SearchState*, float> statecost =
                ExpandToFollowingState(arc, threshold_);
  ...
  void ExpandEpsilonArcs(int max_cycles = std::numeric_limits<int>::max()) {
    ...
    // Generic SSSP algorithm, however care needs to be taken here due to the
...
#endif  // DCD_HMM_CLEVEL_DECODER_H__
