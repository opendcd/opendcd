...
void ParseOptions::RegisterCommon(const std::string &name, T *ptr,
  ...
  if (doc_map_.find(idx) != doc_map_.end())
    LOG(WARN) << "Registering option twice, ignoring second time: " << name;
...
std::string ParseOptions::GetArg(int i) {
  if (i < 1 || i > static_cast<int>(positional_args_.size()))
    LOG(ERROR) << "ParseOptions::GetArg, invalid index " << i;  // code error
  // so use LOG(ERROR)
  ...
  for (i = 1; i < argc; i++) {
    if (std::strncmp(argv[i], "--", 2) == 0) {
      ...
      if (!SetOption(key, value, has_equal_sign)) {
        PrintUsage(true);
        LOG(FATAL) << "Invalid option " << argv[i];
      }
    } else {
      break;
    }
...
void ParseOptions::PrintConfig(std::ostream &os) {
  ...
  for (it = doc_map_.begin(); it != doc_map_.end(); ++it) {
    ...
    if (bool_map_.end() != bool_map_.find(key)) {
      ...
      LOG(FATAL) << "PrintConfig: unrecognized option " << key << "[code error]";
    }
    os << '\n';
  }
  os << '\n';
...
void ParseOptions::ReadConfigFile(const std::string &filename) {
  ...
  if (!is.good()) {
    LOG(FATAL) << "Cannot open config file: " << filename;
  }
  ...
  while (std::getline(is, line)) {
    ...
    if (!SetOption(key, value, has_equal_sign)) {
      PrintUsage(true);
      LOG(ERROR) << "Invalid option " << line << " in config file " << filename;
    }
  }
}
...
void ParseOptions::SplitLongArg(std::string in,
  ...
  if (pos == std::string::npos) {  // we allow --option for bools
    ...
    *has_equal_sign = false;
    ...
    PrintUsage(true);
    LOG(ERROR) << "Invalid option (no key): " << in;
...
bool ParseOptions::ToBool(std::string str) {
  ...
  PrintUsage(true);
  LOG(ERROR) << "Invalid format for boolean argument [expected true or false]: "
            << str;
...
int32 ParseOptions::ToInt(std::string str) {
  ...
  if (str.c_str() == end_pos) {
    PrintUsage(true);
    LOG(ERROR) << "Invalid integer option \"" << str << "\"";
  }
  return ret;
}
...
float ParseOptions::ToFloat(std::string str) {
  ...
  if (str.c_str() == end_pos) {
    PrintUsage(true);
    LOG(ERROR) << "Invalid floating-point option \"" << str << "\"";
  }
  return ret;
}
