...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Simulates the online neural net computation single wav input\n"
        "file, and outputs as a matrix the result, with optional\n"
        ...
        "--apply-log=true.\n"
        "\n"
        "Usage:  online2-wav-nnet2-am-compute2 [options] <nnet-in>\n"
        "<wav-filer> <feature-or-loglikes-wspecifier>\n";
    ...
    ifstream input(wav_rspecifier.c_str());
    WaveData wave_data;
    ...
    writer.Write("features", output);
    num_frames += feats.NumRows();

    KALDI_LOG << "Processed data for utterance " << wav_rspecifier;

    KALDI_LOG << "Processed " << num_frames 
