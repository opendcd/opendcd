...
ostream& operator<<(ostream& os, const StdArc& arc) {
  os << arc.ilabel << " " << arc.olabel << " " << arc.weight << " " 
    << arc.nextstate;
  return os;
}
}*/
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage:   decode-faster [options] fst-in arcs-in loglikes-rspecifier words-wspecifier [alignments-wspecifier]\n";
    ...
    fst::StdFst* fst = fst::StdFst::Read(fst_in_filename); //Because this make less sense!
    if (!fst) {
      KALDI_ERR << "Failed to read fst from : " << fst_in_filename;
      exit(-1);
    }

    vector<const fst::StdFst*> arcs;
    zgf::ReadFstArcTypes(arcs_in_filename, &arcs, 1.0f, false);

    fst::ArcExpandFstOptions opts;
    opts.gc = false;
    fst::StdArcExpandFst decode_fst(*fst, arcs, opts);
    
    BaseFloat tot_like = 0.0;
    kaldi::int64 frame_count = 0;
    int num_success = 0, num_fail = 0;
    FasterDecoderMod decoder(decode_fst, decoder_opts);
