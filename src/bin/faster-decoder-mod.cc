...
#include "faster-decoder-mod.h"

namespace kaldi {


FasterDecoderMod::FasterDecoderMod(const fst::ArcExpandFst<fst::StdArc> &fst,
...
void FasterDecoderMod::Decode(DecodableInterface *decodable) {
...
bool FasterDecoderMod::ReachedFinal() {
...
bool FasterDecoderMod::GetBestPath(fst::MutableFst<LatticeArc> *fst_out) {
...
BaseFloat FasterDecoderMod::GetCutoff(Elem *list_head, size_t *tok_count,
...
void FasterDecoderMod::PossiblyResizeHash(size_t num_toks) {
...
BaseFloat FasterDecoderMod::ProcessEmitting(DecodableInterface *decodable, int frame) {
  ...
  BaseFloat best = std::numeric_limits<BaseFloat>::infinity();

  best_elem = NULL;
  int count = 0;
  for (Elem *e = last_toks; e != NULL; e = e->tail, count++) {
    Token *tok = e->val;
    StateId state = e->key;
    
    if (fst_.NumArcs(state) - fst_.NumInputEpsilons(state) > 0) {
      if (!best_elem || tok->weight_.Value() < 
          best) {
        best = tok->weight_.Value();
        best_elem = e;
      }
    }
  }
  ...
  BaseFloat best_token  = std::numeric_limits<BaseFloat>::infinity();
  ...
  if (best_elem) {
    ...
    for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state);
         !aiter.Done();
         aiter.Next()) {
      //TODO: Search for the best token with an arc
    ...
    if (tok->weight_.Value() < weight_cutoff) {  // not pruned.
      ...
      for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state);
        ...
        if (arc.ilabel != 0) {  // propagate..
          //NOTE: Why does create Weight then change it straightaway
          //to a float in the dollowing line??
          //const Arc& farc = fst_.FstArc(state);
          //StateId arcstate = fst_.ArcState(state);
          //Label arclabel = fst_.ArcLabel(state);
...
void FasterDecoderMod::ProcessNonemitting(BaseFloat cutoff) {
  // Processes nonemitting arcs for one frame. 
  KALDI_ASSERT(queue_.empty());
  for (Elem *e = toks_.GetList(); e != NULL;  e = e->tail)
    queue_.push_back(e->key);
  KALDI_VLOG(2) << "Initial queue size " << queue_.size();
  while (!queue_.empty()) {
  ...
  KALDI_VLOG(2) << "End queue size";
}

void FasterDecoderMod::ClearToks(Elem *list) {
