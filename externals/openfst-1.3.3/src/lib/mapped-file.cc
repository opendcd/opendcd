...
MappedFile::~MappedFile() {
  if (region_.size != 0) {
    if (region_.mmap != NULL) {
      ...
      if (!::UnmapViewOfFile(region_.mmap)) {
        LOG(ERROR) << "failed to UnmapViewOfFile region: "<< strerror(errno);
      }
      if (!::CloseHandle(region_.map_handle)) {
         LOG(ERROR) << "failed to CloseHandle map_handle: "<< strerror(errno);
      }
      if (!::CloseHandle(region_.map_handle)) {
         LOG(ERROR) << "failed to CloseHandle file_handle: "<< strerror(errno);
      }
    } else {
...
MappedFile* MappedFile::Map(istream* s, const FstReadOptions &opts,
  ...
  if (opts.mode == FstReadOptions::MAP && pos >= 0 &&
    ...
    wstring wsource;
    UTF8ToUTF16(opts.source, &wsource);
    HANDLE handle = ::CreateFileW(wsource.c_str(), GENERIC_READ, FILE_SHARE_READ,
                                0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    if (handle != INVALID_HANDLE_VALUE) {
      int pagesize = 0;//getpagesize();
      off_t offset = pos;// % pagesize;
      off_t upsize = size + offset;      
      HANDLE map = ::CreateFileMapping(handle, 0, PAGE_READONLY, 0, 0, 0);      
      if (map != NULL && map != INVALID_HANDLE_VALUE) {
        char *data = reinterpret_cast<char*>(::MapViewOfFile(map, FILE_MAP_READ, 0, 0, 0));
        MemoryRegion region;
        region.mmap = map;
        region.size = upsize;
        region.data = reinterpret_cast<void*>(data + offset);
        region.map_handle = map;
